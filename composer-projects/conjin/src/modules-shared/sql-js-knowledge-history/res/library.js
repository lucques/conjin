...
{   
    db.then(function(db) {
        ...
        if (results.length === 0 || results.every(result => result.values.length === 0)) {
            document.getElementById(divId).innerHTML = '<p>(keine Einträge)</p>';
            return;
        }
        ...
        for (const result of results)
        {
            ...
            for (const row of result.values)
            {
                ...
                if (timeUnit == 'y') {
                    ...
                    const monthTo = dateEnd.match(/-?\d{1,}-\d{1,2}/)[0];
