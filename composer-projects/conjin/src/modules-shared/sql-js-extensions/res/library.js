...
function sql_js_inline_resultsToHTML(results, title = '', truncated = false)
{
    ...
    for (const result of results)
    {
        ...
        {
            content += `<th>${sql_js_inline_escapeHTML(colName)}</th>`;
        }
        content += `
                </tr>
            </thead>
        ...
        for (const row of result.values)
        {
            ...
            {
                content += `<td>${sql_js_inline_escapeHTML(cell)}</td>`;
            }
            content += `</tr>`;
        }
        if (truncated) {
...
{
    let content = `<div class="d-inline-grid" style="grid-template-columns: auto auto auto auto; align-items: baseline;">`;
    ...
    for (const table of tables)
    {
        ...
        content += `
            <div style="justify-self:end;">
                <strong>${sql_js_inline_escapeHTML(table.name)}</strong>
            </div>
        ...
        if (showButton && table.result !== undefined) {
            content += `
                <button id="${collapseButtonId}" class="btn btn-link p-0" type="button" onClick="sql_js_inline_resultsAndSchemaToHTML_aux_toggle('${collapseButtonId}', '${tbodyId}')">
                    <i class="bi ${table.open === true ? 'bi-chevron-down' : 'bi-chevron-right'}"></i>
                </button>
            `;
        }
        ...
        {
            if (col.type === 'data') {
                let colContent = sql_js_inline_escapeHTML(col.name);
        ...
        if (table.result !== undefined) {
            for (const row of table.result.values)
            {
                ...
                for (const col of table.columns) {
                    if (col.type === 'data') {
                        content += `<td style="border: 1px solid #555;">${sql_js_inline_escapeHTML(row[index])}</td>`;
                        index++;
                    }
                    else if (col.type === 'ellipsis') {
...
function sql_js_inline_escapeHTML(value) {
    return String(value)
        .replaceAll('&', '&amp;')
        .replaceAll('<', '&lt;')
        .replaceAll('>', '&gt;')
