<?
    ...
    function ex_start($title = null, bool $numbered = true, $variant = 'exercise', $open = false, $class = '', $style = '') {
        // Track state
    ...
    function ex_sol($restrict_to_groups = [], $restrict_to_users = [], $title = 'Lösungsvorschlag', $variant = 'solution', $open = false) {
        // Track state
        assert(in_array($GLOBALS['exercise_last_macro'], ['start', 'sol_print', 'sol_omit']));
    ...
    function ex_aux_end_last_item() {
        if ($GLOBALS['exercise_last_macro'] == 'sol_omit') {
            // Throw away
            ob_end_clean();
        }
        else {
