<?
    ...
    class IssueTracker {
        ...
        private function may_administer_issue(array $issue): bool {
            ...
            return $this->config->may_edit_own_content_within_n_minutes > 0 &&
                $issue['first_post_author'] === auth_get_user_name() &&
                $created_on !== false &&
        ...
        private function may_administer_issue_by_id(int $issue_id): bool {
            ...
            $query = $this->db->prepare(
                'SELECT first_post_author, first_post_created_on FROM issue WHERE id = ?'
            );
        ...
        private function may_administer_post(array $post): bool {
            ...
            return $this->config->may_edit_own_content_within_n_minutes > 0 &&
                $post['author'] === auth_get_user_name() &&
                $created_on !== false &&
                time() <= $created_on + $this->config->may_edit_own_content_within_n_minutes * 60;
        }
        ...
        private function may_administer_post_by_id(int $issue_id, int $post_id): bool {
            ...
            $query = $this->db->prepare(
                'SELECT author, created_on FROM post WHERE id = ? AND issue_id = ?'
            );
    ...
    CREATE TABLE IF NOT EXISTS "issue" (
        ...
        "first_post_author" TEXT NOT NULL,
    ...
    SQL;
        ...
        private function fetch_issues_with_posts($where = '', $order_by = '', $limit = '') {
            ...
            $query .= 'issue.first_post_author, issue.last_post_status ';
            $query .= 'FROM issue ';
            ...
            foreach ($result as $row) {
                $issues_to_posts[$row['id']] = [
                    ...
                    'first_post_author' => $row['first_post_author'],
                ...
                if (
                    ...
                    if (auth_register_check_postdedup_uuid()) {
                        if ($_POST['content'] !== '') {
                            ...
                            $status       = 'open';
                            $author       = auth_get_user_name();

                            $author_email = isset(auth_get_user()['contents']['email']) ? auth_get_user()['contents']['email'] : null;

                            // Insert issue
                            $query = $this->db->prepare("INSERT INTO issue (summary, last_post_status, last_post_created_on, first_post_created_on, first_post_author) VALUES (?, ?, ?, ?, ?)");
                            $query->execute([$content, $status, $date, $date, $author]);
                            ...
                            $query->execute([$issue_id, $date, $author, $author_email, $content, $status]);
        ...
        function handle_delete_post(): ?HandleDeletePostResult {
            ...
            try {
                ...
                $query = $this->db->prepare(
                    'SELECT created_on, author FROM post WHERE issue_id = ? '
                ...
                if ($issue_deleted) {
                    ...
                    $query = $this->db->prepare(
                        'UPDATE issue SET first_post_created_on = ?, first_post_author = ?, '
...
?>
                    ...
                    <td class="text-truncate" style="max-width:<?= $this->config->view->author_column_width ?>;">
                        <span data-bs-toggle="tooltip" title="<?= htmlspecialchars($post['author'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>" data-bs-placement="left">
                            <?= htmlspecialchars($post['author'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>
                        </span>
                    </td>
